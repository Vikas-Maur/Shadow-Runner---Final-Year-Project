...
# ---- SOUND PLAYER ----
@onready var portal_sound_player: AudioStreamPlayer = $PortalSoundPlayer

# ---- SOUND ARRAY ----
@export var portal_activate_sounds: Array[AudioStream] = []
...
func _activate_portal() -> void:
	...
	# 🔊 Play sound AND change level at same time — no awaiting
	if not portal_activate_sounds.is_empty():
		SoundManager.play_sound(portal_activate_sounds.pick_random())
