...
# ✅ NEW: tick this ON in the Inspector for the fall/void killzone only
@export var is_instant_kill_zone: bool = false

var _damage_cooldowns: Dictionary = {}

func _on_body_entered(body):
	# ✅ If this is the fall killzone, instantly kill the player with fall sound
	if is_instant_kill_zone:
		if body.has_method("die_from_killzone"):
			body.die_from_killzone()
		return
