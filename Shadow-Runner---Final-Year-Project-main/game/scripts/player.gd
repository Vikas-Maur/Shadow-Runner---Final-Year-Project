...
# ---- SOUND PLAYERS ----
@onready var sword_attack_player: AudioStreamPlayer = $SwordAttackPlayer
@onready var shoot_player: AudioStreamPlayer = $ShootPlayer
@onready var stomp_player: AudioStreamPlayer = $StompPlayer
...
@onready var killzone_death_player: AudioStreamPlayer = $KillzoneDeathPlayer
...
@export var stomp_sounds: Array[AudioStream] = []
@export var jump_sounds: Array[AudioStream] = []
@export var damage_sounds: Array[AudioStream] = []
@export var death_sounds: Array[AudioStream] = []
@export var killzone_death_sounds: Array[AudioStream] = []
...
var _died_from_killzone: bool = false
...
func _handle_footsteps(delta: float) -> void:
	if is_dead() or _is_dashing() or stomp_active:
		_footstep_timer = 0.0
		return
...
func _play_footstep() -> void:
	print("footstep called, sounds empty: ", footstep_sounds.is_empty())
...
func _do_jump() -> void:
	...
	coyote_timer = 0.0
	stomp_active = false
	_play_sound(jump_player, jump_sounds)  # 🔊 jump sound
...
func _start_shoot(direction: float) -> void:
	...
	_play_sound(shoot_player, shoot_sounds)  # 🔊 shoot sound
...
func _start_sword_attack() -> void:
	...
	_update_attack_visuals()
	_process_sword_hits()
	_play_sound(sword_attack_player, sword_attack_sounds)  # 🔊 sword sound
...
func _spawn_stomp_shockwave(origin: Vector2) -> void:
	...
	_play_sound(stomp_player, stomp_sounds)  # 🔊 stomp sound
...
func apply_damage(damage_input: Variant, _source: Node = null):
	...
	if current_health == 0:
		...
		LeaderBoard.update_score(GameData.current_score)
		_prepare_death_state(_source)
		_died_from_killzone = false
		_play_sound(death_player, death_sounds)  # 🔊 enemy death sound
		died.emit()
	else:
		_play_sound(damage_player, damage_sounds)  # 🔊 damage sound

# Called by killzone when player falls off the map
func die_from_killzone() -> void:
	if is_dead():
		return
	_died_from_killzone = true
	current_health = 0
	health_changed.emit(current_health, max_health)
	LeaderBoard.load_score()
	LeaderBoard.update_score(GameData.current_score)
	_prepare_death_state(null)
	_play_sound(killzone_death_player, killzone_death_sounds)  # 🔊 fall death sound
	died.emit()
...
# 🔊 SOUND HELPER
# -------------------------
func _play_sound(player: AudioStreamPlayer, sounds: Array[AudioStream]) -> void:
